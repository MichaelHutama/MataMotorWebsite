...
<body class="bg-gray-50 min-h-screen flex flex-col font-didact text-gray-800">
  ...
  @php
    $paymentItems = [
        ...
        ['qty' => 1, 'name' => 'Kampas Rem Depan Avanza', 'price' => 12000000],
    ];
  ...
  <main class="max-w-5xl mx-auto py-16 px-4 md:px-8 w-full flex-1 flex flex-col items-center">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-24 mb-16 w-full">
      ...
      <div class="flex justify-center md:justify-end">
        <div class="w-full max-w-md bg-gray-50 rounded-[32px] p-8 space-y-6 border border-gray-100 flex flex-col items-center">
          ...
          <div class="w-full space-y-4 font-inter">
            ...
            <div class="flex justify-between items-start gap-4">
              ...
              <span class="text-sm font-bold font-century text-gray-900 whitespace-nowrap">IDR {{ number_format($item['price'], 0, ',', '.') }}</span>
            </div>
            @endforeach
            ...
            <div class="pt-6 border-t border-dashed border-gray-300 space-y-3">
              <div class="flex justify-between text-sm font-bold text-gray-400 uppercase tracking-widest">
                <span>Subtotal</span>
                <span class=" font-century text-gray-900">IDR {{ number_format($subtotal, 0, ',', '.') }}</span>
              </div>
              <div class="flex justify-between text-sm font-bold text-gray-400 uppercase tracking-widest">
                <span>Delivery</span>
                <span class=" font-century text-gray-900">IDR {{ number_format($deliveryFee, 0, ',', '.') }}</span>
              </div>
            </div>
            ...
            <div class="pt-6 border-t border-gray-200 flex justify-between items-center">
              ...
              <span class="text-2xl font-black text-mm-navy font-inter tracking-tighter">IDR {{ number_format($grandTotal, 0, ',', '.') }}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="flex justify-start">
        <div class="w-full max-w-sm bg-white rounded-3xl shadow-sm border border-gray-100 p-8 space-y-6">
          <h2 class="text-lg font-bold text-gray-900 font-inter uppercase tracking-widest mb-6 border-b border-gray-100 pb-4">Payment Method</h2>
          ...
          <div class="space-y-4 font-inter">
            ...
            <div class="space-y-3">
              <label class="flex items-center gap-4 p-4 rounded-2xl bg-gray-50 border-2 border-transparent has-[:checked]:border-mm-navy has-[:checked]:bg-white transition-all cursor-pointer group">
                ...
                <div class="flex-1">
                  <span class="block text-sm font-bold text-gray-900 uppercase">QRIS</span>
                </div>
              </label>
            ...
            <div class="space-y-3">
              <label class="flex items-center gap-4 p-4 rounded-2xl bg-gray-50 border-2 border-transparent has-[:checked]:border-mm-navy has-[:checked]:bg-white transition-all cursor-pointer group">
                ...
                <div class="flex-1">
                  <span class="block text-sm font-bold text-gray-900 uppercase">Bank Transfer</span>
                </div>
              </label>
            ...
            <label class="flex items-center gap-4 p-4 rounded-2xl bg-gray-50 border-2 border-transparent has-[:checked]:border-mm-navy has-[:checked]:bg-white transition-all cursor-pointer group">
              ...
              <div class="flex-1 text-sm font-bold text-gray-900 uppercase">Cash on Delivery (COD)</div>
            </label>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        payBtn.addEventListener('click', function() {
            ...
            confirmModal(
                'Confirm Payment?',
                `You are about to pay using <b>${methodLabel}</b>. Please ensure your balance is sufficient.`,
                'YES',
                'NO',
                'info',
                function() {
                        window.location.href = "{{ route('customer-paymentsuccess') }}";
                }
            );
        });
    });
