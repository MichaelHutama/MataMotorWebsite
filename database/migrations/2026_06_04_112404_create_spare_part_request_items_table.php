...
public function up(): void
{
    Schema::create('spare_part_request_items', function (Blueprint $table) {
        $table->string('SparePartRequestID', 80);
        $table->string('SparePartID', 20);
        $table->integer('Amount')->nullable();
        $table->primary(['SparePartRequestID', 'SparePartID']); // composite PK
        $table->timestamps();

        $table->foreign('SparePartRequestID')
              ->references('SparePartRequestID')->on('spare_part_requests')
